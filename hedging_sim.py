...
def simulate_delta_hedging(S0, K, T, r, sigma, option_type='call', steps=50):
    """
    Simulate a delta hedging strategy over the option lifetime using
    a discrete time binomial price path and adjusting delta hedge each step.
    """
    dt = T / steps
    prices = [S0]
    np.random.seed(42)
    ...
    for i, t_step in enumerate(np.linspace(T, 0, steps + 1)):
        ...
        # Hedge adjustment: number of shares needed
        if i == 0:
            shares_held = delta
        ...
        # Portfolio value = value of shares + cash - option payoff at maturity
...
def simulate_covered_call(S0, K, T, r, sigma, steps=50):
    """
    Simulate a covered call strategy:
    - Buy underlying asset
    - Sell call option
    Track portfolio value over time using GBM price simulation
    """
    dt = T / steps
    prices = [S0]
    np.random.seed(42)
    ...
    # Initial portfolio: long 1 stock, short 1 call option
    ...
    for i, t_step in enumerate(np.linspace(T, 0, steps + 1)):
        S = prices[i]
        time_to_exp = max(t_step, 0.0001)  # avoid zero
        opt = Option(S, K, time_to_exp, r, sigma, 'call')
        option_price = opt.price()

        # Portfolio = stock price - option price + option premium received (initially)
